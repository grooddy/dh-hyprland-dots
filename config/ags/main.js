...
var { Astal, Gtk, GLib, AstalHyprland, AstalTray, Gio } = imports.gi;
...
function Clock() {
  ...
  GLib.timeout_add(GLib.PRIORITY_DEFAULT, 1e3, () => {
    label.label = (/* @__PURE__ */ new Date()).toLocaleTimeString("ru-RU", {
      hour: "2-digit",
      minute: "2-digit"
    });
    return true;
...
var app = new Astal.Application({ instance_name: "tahoe_gtk3" });
app.connect("activate", () => {
  app.hold();
  console.log("--- Tahoe Shell: Starting (GTK3 Mode) ---");
  try {
    GLib.mkdir_with_parents(GLib.path_get_dirname(paths.tempCss), 493);
